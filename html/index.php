...
if ($_SERVER["SERVER_NAME"] === "contact") {
    $deep1 = "usedns";
}
    ...
    case "deals":
        switch ($deep2) {
            case "deals":
                ...
                break;
            case "hot_requests":
                define("MODULE", "deals/hot_requests");
                break;
            case "discount":
                define("MODULE", "deals/discount");
                break;
            case "certificates":
                define("MODULE", "deals/certificates");
                break;
            default:
                define("MODULE", "error");
                break;
    ...
    case "stat":
        switch ($deep2) {
            case "clients":
                ...
                break;
            case "debt":
                define("MODULE", "stat/stat_debt");
                break;
            case "depart":
                define("MODULE", "stat/stat_depart");
                break;
            case "hotels":
                define("MODULE", "stat/stat_hotels");
                break;
            case "salary":
                define("MODULE", "stat/stat_salary");
                break;
            default:
                define("MODULE", "error");
                break;
    ...
    case "dialogs": // Модальные окна
        switch ($deep2) {
            case "ticket_add":
                ...
                break;
            case "airports":
                define("MODULE", $deep1 . "/airports");
                break; //аэропорты
            case "client_search":
                define("MODULE", $deep1 . "/client_search");
                break; //поиск клиента
            case "pay_add":
                define("MODULE", $deep1 . "/pay_add");
                break; //добавление платежа
            case "pay_types":
                define("MODULE", $deep1 . "/pay_types");
                break; //типы платежа
            case "food_types":
                define("MODULE", $deep1 . "/food_types");
                break; //типы питания
            case "room_types":
                define("MODULE", $deep1 . "/room_types");
                break; //типы комнат
            case "place_types":
                define("MODULE", $deep1 . "/place_types");
                break; //типы проживания
            case "client_sources":
                define("MODULE", $deep1 . "/client_sources");
                break; //источники информации
            case "client_add":
                define("MODULE", $deep1 . "/client_add");
                break; //добавление клиента
            case "operators":
            ...
            case "service_add":
                define("MODULE", $deep1 . "/service_add");
                break; //добавление платежа
            case "payment_add":
                define("MODULE", $deep1 . "/payment_add");
                break; //добавление платежа
            case "deals_list":
                define("MODULE", $deep1 . "/deals_list");
                break; //добавление платежа
...
$p->set_file(array(
                 "head" => "_head.ht", //Шапка
                 "foot" => "_foot.ht", //Низ
                 "head_print" => "_head_print.ht", //Шапка печати
                 "foot_print" => "_foot_print.ht", //Низ печати
...
if (isset($_SESSION["str_cookies"])) {
    $p->set_block("head", "reset_time_sa");
    unset($_SESSION["str_cookies"]);
}
...
if (is_array($ERROR_GLOBAL) && !empty($ERROR_GLOBAL)) {
    ...
    foreach ($STATUS_GLOBAL las $value) {
...
if ($auth->CheckAuth()) {
    ...
    $dbo->query("select count(*) new_mails from mails where receiver_id = " . $auth->auth["uid"] . " and readed = 0");
    if ($dbo->next_record()) $new_mails = $dbo["new_mails"];



    $dbo->query("select count(*) active_tasks from tasks where manager_id = " . $auth->auth["uid"] . " and deleted = 0");
    if ($dbo->next_record()) $active_tasks = $dbo["active_tasks"];
    $dbo->query("SELECT count(*) task_num FROM (SELECT 
  d.*,
  CASE WHEN status_id < 3 AND start_date < NOW() THEN 'Клиент улетел, документы не выданы'
  WHEN full_sum - payed_sum_rub > 0 AND start_date < NOW() THEN 'Клиент улетел не оплатил'
  END state
FROM
  deals d 
WHERE d.status_id NOT IN (0, 4, 5, 6) and d.manager_id = ".$auth->auth["user_id"]."
)a WHERE a.state IS NOT NULL");

    if ($dbo->next_record()) $active_tasks += $dbo["task_num"];
    $p->set_var(array(
                    "USER_FIO" => $auth->auth["fio"],
                    "USER_OFFICE" => $auth->auth["office"],
